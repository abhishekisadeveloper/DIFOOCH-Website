        ...
        <div className='flex items-center text-center gap-6 overflow-x-auto snap-x snap-mandatory scroll-smooth no-scrollbar w-full'>
            {TestimonialsData[0].TestimonialsData.map((item) => (
                <div key={item.id} className='flex flex-col snap-start flex-shrink-0 max-w-[100%] md:max-w-[360px] items-center p-8 sm:p-[36.23px] bg-white rounded-3xl text-center'>
                    <img src={item.image} alt="image" className='w-[101px] h-[101px] mb-6 sm:mb-[27.17px]' />
                    <Typography className='mb-0.5 font-semibold sm:text-large sm:leading-large' variant="h2">{item.name}</Typography>
                    <Typography className='text-gray2' variant="p">{item.title}</Typography>
                    <img src={item.line} alt="line" className='w-full h-[1px] mt-6 sm:mt-[27.17px] mb-12 sm:mb-[27.17px]' />
                    <Typography className='text-gray1 mb-3' variant="p">{item.quote}</Typography>
                    <img className='w-[144.90px] h-[27.16px]' src={item.stars} alt="stars" />
                </div>
            ))}
        </div>
    </div>
